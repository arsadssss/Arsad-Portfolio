...
const Projects = () => {
    ...
    const NavItems = ProjectsData.map((v,i)=>{
        return(
            <li key={v.id} className={`border-b-2 text-sm cursor-pointer text-black font-semibold hover:border-[#ff7322] ${ proVal === v.val ? 'border-[#ff7322]' : (proVal == '' ? (v.val === 'EmailTemplate' ? 'border-[#ff7322]' : '') : 'border-b-gray-200')}`} onClick={()=>getLandingPro(v.val)}>{v.name}</li>
        );
    });
